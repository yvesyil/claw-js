...
int main()
{
    ...
    start = clock();
    cl_int err = opencl_setup_context_and_command_queue(&device_id,
            &ctx, &cmd_queue);
    if (err!=CL_SUCCESS) {
    ...
    err = opencl_setup_program(ctx, &prog, &device_id, source1_str, source1_size);
    if (err!=CL_SUCCESS) {
        fprintf(stderr, "%s", opencl_get_error_string(err));
        exit(err);
    }

    err = opencl_setup_buffers_and_kernel(ctx, cmd_queue, prog,
    ...
    start = clock();
    err = opencl_setup_context_and_command_queue(&device_id,
            &ctx, &cmd_queue);
    if (err!=CL_SUCCESS) {
        fprintf(stderr, "%s", opencl_get_error_string(err));
        exit(err);
    ...
    err = opencl_setup_program(ctx, &prog, &device_id, source2_str, source2_size);
    if (err!=CL_SUCCESS) {
        fprintf(stderr, "%s", opencl_get_error_string(err));
        exit(err);
    }
