...
cl_int opencl_setup_program(cl_context ctx, cl_program *prog,
			    cl_device_id *device_id, const char *source_str,
			    size_t source_size)
{
	cl_int err;
	*prog = clCreateProgramWithSource(ctx, 1, &source_str,
	...
	size_t log_size;
	err = clGetProgramBuildInfo(*prog, *device_id, CL_PROGRAM_BUILD_LOG, 0,
				    NULL, &log_size);
	if (err != CL_SUCCESS) {
		return err;
	}
	...
	err = clGetProgramBuildInfo(*prog, *device_id, CL_PROGRAM_BUILD_LOG,
...
cl_int opencl_setup_buffers_and_kernel(cl_context ctx,
				       ...
				       float *A, size_t *shapeA, float *B,
				       size_t *shapeB, float *C, size_t *shapeC)
{
	cl_int err;
	cl_mem a_mem_obj = clCreateBuffer(ctx, CL_MEM_READ_ONLY,
					  shapeA[0] * shapeA[1] * sizeof(float),
					  NULL, &err);
	if (err != CL_SUCCESS) {
		return err;
	}
	...
	cl_mem b_mem_obj = clCreateBuffer(ctx, CL_MEM_READ_ONLY,
					  shapeB[0] * shapeB[1] * sizeof(float),
					  NULL, &err);
	if (err != CL_SUCCESS) {
		return err;
	}
	...
	cl_mem c_mem_obj = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY,
					  shapeC[0] * shapeC[1] * sizeof(float),
					  NULL, &err);
	if (err != CL_SUCCESS) {
		return err;
	}
	...
	err = clEnqueueWriteBuffer(cmd_queue, a_mem_obj, CL_TRUE, 0,
				   shapeA[0] * shapeA[1] * sizeof(float), A, 0,
				   NULL, NULL);
	if (err != CL_SUCCESS) {
		return err;
	}
	...
	err = clEnqueueWriteBuffer(cmd_queue, b_mem_obj, CL_TRUE, 0,
				   shapeB[0] * shapeB[1] * sizeof(float), B, 0,
				   NULL, NULL);
	if (err != CL_SUCCESS) {
		return err;
	}
	...
	cl_uint arg_idx = 0;
	err = clSetKernelArg(kernel, arg_idx++, sizeof(cl_mem),
			     (void *)&a_mem_obj);
	if (err != CL_SUCCESS) {
		return err;
	}
	err = clSetKernelArg(kernel, arg_idx++, sizeof(cl_mem),
			     (void *)&shape_a_mem_obj);
	if (err != CL_SUCCESS) {
		return err;
	}
	err = clSetKernelArg(kernel, arg_idx++, sizeof(cl_mem),
	...
	err = clEnqueueNDRangeKernel(cmd_queue, kernel, 2, NULL,
				     global_witem_size, local_witem_size, 0,
				     NULL, NULL);
	if (err != CL_SUCCESS) {
		return err;
	}
	...
	err = clEnqueueReadBuffer(cmd_queue, c_mem_obj, CL_TRUE, 0,
				  shapeC[0] * shapeC[1] * sizeof(float), C, 0,
				  NULL, NULL);
	if (err != CL_SUCCESS) {
		return err;
	}
